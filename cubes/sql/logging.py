...
class SQLRequestLogHandler(RequestLogHandler):
    def __init__(self, url=None, table=None, dimensions_table=None, **options):
        ...
        try:
            ...
            columns = [
                Column("id", Integer, Sequence(table + "_seq"), primary_key=True),
        ...
        if dimensions_table:
            try:
                self.dims_table = Table(dimensions_table, metadata, autoload_with=self.engine)
                ...
                metadata.create_all(self.engine, tables=[self.dims_table])
        else:
