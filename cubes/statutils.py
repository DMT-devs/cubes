...
def calculators_for_aggregates(cube, aggregates, drilldown_levels=None, split=None):
    ...
    for aggregate in aggregates:
        ...
        try:
            ...
            raise ArgumentError(
                "Unknown post-calculation function '%s' for " "aggregate '%s'" % (aggregate.function, aggregate.name)
            )

        if aggregate.measure:
            source = cube.measure_aggregate(aggregate.measure)
        else:
            raise InternalError("No measure specified for aggregate '%s' in " "cube '%s'" % (aggregate.name, cube.name))
...
def _window_function_factory(aggregate, source, drilldown_paths, split_cell, window_function, label):
    ...
    if window_size is None:
        ...
        raise ModelError(
            "window size for aggregate '%s' sohuld be an integer " "greater than or equeal 1" % aggregate.name
        )
...
CALCULATED_AGGREGATIONS = {
    "wma": partial(
        _window_function_factory, window_function=weighted_moving_average, label="Weighted Moving Avg. of {measure}"
    ),
    "sma": partial(
        _window_function_factory, window_function=simple_moving_average, label="Simple Moving Avg. of {measure}"
    ),
    "sms": partial(_window_function_factory, window_function=simple_moving_sum, label="Simple Moving Sum of {measure}"),
    "smstd": partial(
        _window_function_factory, window_function=simple_stdev, label="Moving Std. Deviation of {measure}"
    ),
    "smrsd": partial(
        _window_function_factory, window_function=simple_relative_stdev, label="Moving Relative St. Dev. of {measure}"
    ),
