...
def assert_instance(obj, class_, label):
    ...
    if not isinstance(obj, class_):
        raise ModelInconsistencyError(
            "%s should be sublcass of %s, " "provided: %s" % (label, class_.__name__, type(obj).__name__)
        )
...
class MissingPackage(object):
    ...
    def _fail(self):
        ...
        raise MissingPackageError(
            ...
            "Please install the package%s%s%s" % (self.package, source, use, comment)
        )
...
def coalesce_option_value(value, value_type, label=None):
    ...
    try:
        ...
        raise ArgumentError("Unable to convert %svalue '%s' into type %s" % (label, "", value_type))
    return return_value
