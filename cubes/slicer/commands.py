...
@click.group()
@click.pass_context
@click.option("--debug/--no-debug", envvar="CUBES_DEBUG", default=False, help="Enable/disable debugging output")
def cli(ctx, debug):
    ...
    if extension_name:
        ...
        if extension.options:
            ...
            for option in extension.options.values():
                ...
                click.echo("    {name} ({type_}){desc}".format(name=name, desc=desc, type_=type_))
        else:
...
@cli.command()
@click.option("--verbose/--terse", "verbose", default=False, help="Display also cube description")
@click.argument("config", required=False, default=DEFAULT_CONFIG, type=click.Path(exists=True))
@click.pass_context
...
@click.option("--warnings/--no-warnings", "show_warnings", default=True, help="enable/disable warnings")
    ...
    if error_count == 0:
        if warning_count == 0:
            if default_count == 0:
                ...
                verdict = "model can be used, " "make sure that the defaults reflect reality"
        else:
...
@cli.command()
@click.option("--aggregate", is_flag=True, default=False, help="Test aggregate of whole cube")
...
def test(aggregate, exclude_stores, include_stores, config, cube):
    ...
    for name in cube_list:
        ...
        try:
            browser.test(aggregate=aggregate)
...
@model.command()
@click.option(
    "--format", "model_format", type=click.Choice(["json", "bundle"]), default="json", help="output model format"
)
@click.option("--force", is_flag=True, default=False, help="replace existing model bundle")
@click.argument("model_path", metavar="MODEL")
...
@cli.group()
@click.pass_context
@click.option("--store", nargs=1, help="Name of the store to use other than default. Must be SQL.")
@click.option("--config", nargs=1, default=DEFAULT_CONFIG, help="Name of slicer.ini configuration file")
...
@click.option("--materialize", "-m", is_flag=True, default=False, help="create materialized view (table)")
@click.option("--index/--no-index", default=True, help="create index for key attributes")
@click.option("--schema", "-s", help="target view schema (overrides default fact schema")
...
def denormalize(ctx, force, materialize, index, schema, cube, target):
    ...
    if cube:
        ...
        targets = [store.naming.denormalized_table_name(name) for name in names]
    ...
    for cube_name, target in cubes:
        ...
        print("denormalizing cube '%s' into '%s'" % (cube_name, target))
...
@click.option("--index/--no-index", default=True, help="create index for key attributes")
@click.option("--schema", "-s", help="target view schema (overrides default fact schema")
@click.option("--dimension", "-d", "dimensions", multiple=True, help="dimension to be used for aggregation")
@click.argument("cube", required=False)
@click.argument("target", required=False)
@click.pass_context
def sql_aggregate(ctx, force, index, schema, cube, target, dimensions):
    ...
    if cube:
        ...
        names = workspace.cube_names()
        targets = [store.naming.aggregated_table_name(name) for name in names]
        cubes = zip(names, targets)
    ...
    for cube_name, target in cubes:
        ...
        store.create_cube_aggregate(
            cube, target, replace=force, create_index=index, schema=schema, dimensions=dimensions
        )
...
@cli.command()
@click.option("--config", type=click.Path(exists=True), required=False, default=DEFAULT_CONFIG)
@click.option("--aggregate", "-a", "aggregates", multiple=True, help="List of aggregates to get")
@click.option("--cut", "-c", "cuts", multiple=True, help="Cell cut")
@click.option("--split", "split_str", multiple=False, help="Split cell")
@click.option("--drilldown", "-d", "drilldown", multiple=True, help="Drilldown dimensions")
@click.option("--on-row", "on_rows", multiple=True, help="Attribute to put on row (default is all)")
@click.option("--on-column", "on_columns", multiple=True, help="Attribute to put on column (default is none)")
@click.option("--format", "-f", "formatter_name", default="cross_table", help="Output format")
@click.argument("cube_name", metavar="CUBE")
@click.pass_context
def aggregate(ctx, config, cube_name, aggregates, cuts, drilldown, formatter_name, split_str, on_rows, on_columns):
    """Aggregate a cube"""
    config = read_config(config)
    ...
    result = browser.aggregate(
        cell, aggregates=aggregates, drilldown=drilldown, split=split, page=None, page_size=None, order=None
    )

    if formatter_name:
        formatter = ext.formatter(formatter_name)
        output = formatter.format(
            browser.cube, result, onrows=on_rows, oncolumns=on_columns, aggregates=aggregates, aggregates_on="columns"
        )
    else:
...
def members(ctx, config, cube_name, cuts, dim_name, output_format):
    ...
    # TODO: pagination
    values = browser.members(cell, dimension, depth=depth, hierarchy=hierarchy, page=None, page_size=None)
