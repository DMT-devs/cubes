...
from ..errors import UserError, ConfigurationError, InternalError
from ..formatters import csv_generator
from .. import ext
...
def _store_option(config, option, default, type_=None, allowed=None, section="server"):
    ...
    if allowed and value not in allowed:
        raise ConfigurationError("Invalued value '%s' for option '%s'" % (value, option))
...
def initialize_slicer(state):
    ...
    with state.app.app_context():
        ...
        if not current_app.slicer.authenticator and workspace.authorizer:
            logger.warn("No authenticator specified, but workspace seems to " "be using an authorizer")
...
def aggregate(cube_name):
    ...
    output_format = validated_parameter(request.values, "format", values=["json", "csv"], default="json")

    header_type = validated_parameter(request.values, "header", values=["names", "labels", "none"], default="labels")
    ...
    if header_type == "names":
        ...
        header = []
        for label_for in result.labels:
            # TODO: add a little bit of polish to this
            if label_for == SPLIT_DIMENSION_NAME:
                header.append("Matches Filters")
            else:
                header += [attr.label or attr.name for attr in cube.get_attributes([label_for], aggregated=True)]
    else:
        header = None

    fields = result.labels
    generator = csv_generator(result, fields, include_header=bool(header), header=header)
...
def cube_facts(cube_name):
    ...
    # Get the result
    facts = g.browser.facts(g.cell, fields=fields, order=g.order, page=g.page, page_size=g.page_size)
...
def cube_members(cube_name, dimension_name):
    ...
    if depth and level:
        raise RequestError("Both depth and level provided, use only one " "(preferably level)")
    ...
    values = g.browser.members(g.cell, dimension, depth=depth, hierarchy=hierarchy, page=g.page, page_size=g.page_size)
...
def cube_search(cube_name):
    ...
    search_engine = cubes_search.create_searcher(engine_name, browser=g.browser, locales=g.locales, **options)
