...
__all__ = ("Authorizer", "SimpleAuthorizer", "AuthorizationError", "NotAuthorized", "right_from_dict")
...
class _SimpleAccessRight(object):
    ...
    def __init__(self, roles, allowed_cubes, denied_cubes, cell_restrictions, hierarchy_limits):
...
class SimpleAuthorizer(Authorizer):
    __options__ = [
        {"name": "rights_file", "description": "JSON file with access rights", "type": "string"},
        {"name": "roles_file", "description": "JSON file with access right roles", "type": "string"},
        {
            "name": "order",
        ...
        if ident_dim:
            ...
            if len(hier) != 1:
                raise ConfigurationError(
                    "Identity hierarchy has to be flat " "(%s in dimension %s is not)" % (str(hier), str(ident_dim))
                )
